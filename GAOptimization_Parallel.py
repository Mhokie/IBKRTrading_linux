...
def GAFunc(index):
    ...
    for i in range(index1, index2):
        ...
        varbound=np.array([[DEMA_SMA_1.min(),DEMA_SMA_1.max()],[DiffDoubleDEMA_SMA_1.min(),DiffDoubleDEMA_SMA_1.max()],[DEMA_SMA_4.min(),DEMA_SMA_4.max()],[DEMA_SMA_1.min(),DEMA_SMA_1.max()],[DiffDoubleDEMA_SMA_1.min(),DiffDoubleDEMA_SMA_1.max()], [-0.035, 0.0]])

        model = ga1(function=f,dimension=6,variable_type='real',variable_boundaries=varbound,algorithm_parameters=algorithm_param)
        ...
        if (abs(f(model.best_variable)) > abs(f([Parameters[STK]['T0'], Parameters[STK]['T1'],Parameters[STK]['T2'], Parameters[STK]['T3'],Parameters[STK]['T4'], Parameters[STK]['Max%Loss']])) and (Positions[STK][Positions.index[-1]] == 0.0)):
            ...
            Parameters[STK]['Max%Loss'] = model.best_variable[5]
